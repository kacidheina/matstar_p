@extends('layouts.app')
@section('title') Debitet e Klienteve @endsection
                ...
                <div class="portlet light bordered">
                    <div class="portlet-title">
                        <div class="caption font-dark">
                            ...
                            <span class="caption-subject bold uppercase">Debitet e Klienteve</span>
                        </div>
                        <div class="tools">
                    ...
                    <div class="portlet-body">
                        <table class="table table-striped table-bordered table-hover" id="debts_table">
                            ...
                            <tbody>
                            @foreach($clients_debts as $key => $clients_debt)
                                <tr id="idRow{{$clients_debt->id}}">
                                    <td> {{$key+1}}</td>
                                    <td> {{$clients_debt->debitor_client    }}</td>
                                    <td> {{$clients_debt->sum}}</td>
                                    <td id="status{{$clients_debt->id}}"> @if($clients_debt->status == 'unpaid') <button class="btn btn-danger">PaPaguar</button> @else @if($clients_debt->datePaymentClient != '0000-00-00 00:00:00')<button class="btn btn-success">I Paguar <br> Me :{{ $clients_debt->datePaymentClient }}</button> @endif @endif</td>
                                    <td> {{$clients_debt->dateDebt}} </td>
                                    <td><p hidden>{{$clients_debt->created_at}}</p> Nga: <b>{{$clients_debt->creator->name}}</b> <br> Me: <b>{{date('g:i a, F j, Y',strtotime($clients_debt->created_at))}}</b> </td>
                                    <td>
                                        <p hidden>{{$clients_debt->updated_at}}</p>
                                        Nga:
                                        @if($clients_debt->modifier != null or $clients_debt->modifier != 0)
                                            <b>{{$clients_debt->modifier->name}}</b>
                                        @else
                                            <b>Pa Modifikuar</b>
                                        @endif

                                        <br> Me:
                                        @if($clients_debt->updated_at != null or $clients_debt->updated_at != "0000-00-00 00:00:00")
                                            <b>{{date('g:i a, F j, Y',strtotime($clients_debt->updated_at))}}</b>
                                        @else
                                            <b>Pa Modifikuar</b>
                                        @endif
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                {{--<li><a class="show_descr_debt" data-href-descr="{{url('view_client_debit',$clients_debt->id)}}" data-id-get = {{ $clients_debt->id }}  ><i class="fa fa-eye"></i>Shiko Shenim</a></li>--}}
                                                {{--<li><a data-get-id="{{$clients_debt->id}}" data-href="{{url('update_debit',$clients_debt->id)}}" class="modify_debt"><i class="fa fa-edit"></i>Ndrysho</a></li>--}}
                                                {{--<li><a class="delete_debt" data-id = {{ $clients_debt->id }}  ><i class="fa fa-trash"></i>Fshij</a></li>--}}
